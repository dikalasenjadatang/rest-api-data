        ...
        try:
            ...
            for item in response.get('items', []):
                ...
                comments = [{
                    ...
                    'gender': determine_gender(comment['snippet']['topLevelComment']['snippet']['authorDisplayName'])
...
def determine_gender(name):
    # Simple gender determination based on common name prefixes
    male_prefixes = ['mr', 'mr.', 'sir', 'he', 'him', 'his']
    female_prefixes = ['ms', 'ms.', 'mrs', 'mrs.', 'miss', 'she', 'her']
    ...
    # If no prefix is found, use genderize.io API as a fallback
    try:
        response = requests.get(f'https://api.genderize.io/?name={name.split()[0]}')
        data = response.json()
        if data['gender']:
            return data['gender']
    except:
